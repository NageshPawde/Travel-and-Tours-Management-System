...
public class Bookhotel extends JFrame implements ActionListener{
    ...
    public Bookhotel(String username){
        ...
        add(l4);
         ...
         c1 = new Choice();
        c1.add("ITC Grand HOTEL");
        c1.add("The IMPERIAL Hotel");
        c1.add("THE LEELA PALACE");
        c1.add("RADDISON HOTEL");
        c1.add("TAJ HOTEL");
        c1.add("THE BAY CLUB");
        c1.add("ELSEWHERE HOTEL");
        c1.add("QUICKSTOP HOTEL");
     ...
     public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1){
            ...
            int cost =0;
            if(p.equals("ITC Grand HOTEL")){
                cost +=2000;
                
            }
            else if(p.equals("The IMPERIAL Hotel")){
                cost +=2000;
        }
            else if(p.equals("THE LEELA PALACE")){
                cost +=2000;
            }else if(p.equals("RADDISON HOTEL")){
                cost +=2000;
            }
            else if(p.equals("TAJ HOTEL")){
                cost +=2000;
            }
            else if(p.equals("THE BAY CLUB")){ 
    
                cost +=2000;
            }
            else if(p.equals("ELSEWHERE HOTEL")){
                cost +=2000;
            }
            else if(p.equals("QUICKSTOP HOTEL")){
                cost +=2000;
            }
            ...
            String q ="insert into bookhotel values('"+username+"','"+hotel+"','"+rooms+"','"+id+"','"+number+"','"+phone+"','"+price+"')";
            try{
                c.s.executeUpdate(q);
                JOptionPane.showMessageDialog(null,"Hotel Book Successfully!");
            }catch(Exception e){
